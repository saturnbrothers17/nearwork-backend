...
async function run() {
  console.log('=== WORKERS IN TURSO ===');
  const workers = await query('SELECT * FROM "WorkerProfile"');
  console.log(workers);

  console.log('\n=== USERS IN TURSO ===');
  const users = await query('SELECT id, name, phone, role FROM "User"');
  console.log(users);

  console.log('\n=== BOOKINGS IN TURSO ===');
  const bookings = await query('SELECT id, bookingNumber, status, customerId, workerId, serviceId, totalAmount, createdAt FROM "Booking" ORDER BY createdAt DESC');
  console.log(bookings);

  console.log('\n=== SERVICES IN TURSO ===');
  const services = await query('SELECT id, name, basePrice, categoryId FROM "Service"');
